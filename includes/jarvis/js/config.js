	...
	**/
	...
	function isMobile() {
		...
		var ismobile = (/iphone|ipad|ipod|chrome|mozilla|gecko|android|blackberry|mini|windows\sce|palm/i.test(navigator.userAgent.toLowerCase()));	
	...
	function setup_flots() {
		...
		if ($("#sin-chart").length) {
				...
				sin.push([5, Math.sin(75)]);
				sin.push([6, Math.sin(135)]);
	...
	function setup_all_buttons() {
		...
		$('#theme-switcher ul#theme-links-js li a').bind('click',
			function(e) {
				$("#switch-theme-js").attr("href", "/includes/jarvis/css/themes/"+$(this).data('rel')+".css");
				return false;
			}
		);
	...
	function setup_chatbox_demo() {
		...
		$('#send-msg-js').click(function() {
			...
			if (msg_input.length) {
				var msg_input = $.istying.val();
				id++;
				$('.tab-pane.active > div > .chat-messages').prepend('<p id="message-dynamic-' + id + '" class="message-box you"><img src="/includes/jarvis/img/avatar/avatar_0.jpg" alt=""><span class="message"><strong>Me</strong><span class="message-time">by Victoria at 14:25pm, 4th Jan 2013</span><span class="message-text">' + msg_input + '</span></span></p>')
